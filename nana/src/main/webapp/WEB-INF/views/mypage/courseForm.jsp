...
function boardValidate(){
	...
	if(confirm("처음 코스를 등록하면 제목을 변경하지 못합니다. 그대로 진행하시겠습니까?")){
		return true;
	} else {
		return false;
	}
...
<main>
                ...
                <div class="row col-xs-12" style="padding: 0 40px;">
                    <div class="col course-settings">
                        ...
                        <form:form 
                        	class="col" 
	                        name="courseFrm"
	                        action="${pageContext.request.contextPath}/mypage/courseEnroll?${_csrf.parameterName}=${_csrf.token}"
							method="post"
							enctype="multipart/form-data"
							onsubmit="return boardValidate();">
							<input type="hidden" name="id" value="${principal.id}">
                            <label for=""><strong>여행 코스 제목</strong></label>
                            <input type="text" class="course-title" id="course-title" name="title" placeholder="제목을 입력하세요." />
                            <p id="check-title" style="color: red;"></p>
                            <label for=""><strong>여행 장소</strong></label>
                            <input type="text" class="course-name" id="course-name" name="travelLocal" value="" placeholder="여행 장소를 위의 리스트에서 추가하세요." readonly/>
                            ...
                            <textarea name="content" id="subject" placeholder="일정을 넣어보세요."></textarea>
    ...
    <script>
        ...
        $("#course-title").blur(function() {
        	var $title = $('#course-title').val();
        	
        	$.ajax({
        		url : '${pageContext.request.contextPath}/mypage/titleCheck?title='+$title,
        		type : 'get',
        		success : function(data) {						
 
        			if (data == 1) {
        					$("#check-title").text("이미 있는 제목입니다. 수정해주세요.");
        					$("input.course-submit").attr("disabled", true);
        				} else if(data == 0) {
        					$("#check-title").text("");
        					$("input.course-submit").attr("disabled", false);
            			}
